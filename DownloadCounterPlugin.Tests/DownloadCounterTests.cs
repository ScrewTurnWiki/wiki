namespace DownloadCounterPlugin.Tests
{
	using System;
	using NUnit.Framework;
	using Rhino.Mocks;
	using ScrewTurn.Wiki.PluginFramework;
	using ScrewTurn.Wiki.Plugins.PluginPack;
...
<countDownloads pattern=""#CoUnT#-#DaIlY#-#WeEkLy#-#MoNtHlY#"" startDate=""{0:yyyy'/'MM'/'dd}"">
	...
	<attachment name=""attn.zip"" page=""inexistent-page"" />
			...
			Rhino.Mocks.Constraints.Is.Matching(
				( StDirectoryInfo dir ) => dir.FullPath == "/my/" ) ).Return( myFiles ).Repeat.Times( 2 );

			// /my/other/*
			Expect.Call( host.ListFiles( null ) ).IgnoreArguments( ).Constraints(
				Rhino.Mocks.Constraints.Is.Matching(
					( StDirectoryInfo dir ) => dir.FullPath == "/my/other/" ) ).Return( myOtherFiles ).Repeat.Times( 2 );
