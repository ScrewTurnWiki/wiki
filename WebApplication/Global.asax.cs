...
namespace ScrewTurn.Wiki
{
	...
	public class Global : HttpApplication
	{
		...
		private void LogError( Exception ex )
		{
			...
			try
			{
				Log.LogEntry( Tools.GetCurrentUrlFixed( ) + "\n" +
					ex.Source + " thrown " + ex.GetType( ).FullName + "\n" + ex.Message + "\n" + ex.StackTrace,
					EntryType.Error, Log.SystemUsername );
			}
			catch { }
		}
		...
		protected void Application_Error( object sender, EventArgs e )
		{
			...
			if ( !Request.PhysicalPath.ToLowerInvariant( ).Contains( "error.aspx" ) ) UrlTools.Redirect( "Error.aspx" );
		}
