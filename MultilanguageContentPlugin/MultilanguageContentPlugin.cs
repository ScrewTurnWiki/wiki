...
namespace ScrewTurn.Wiki.Plugins.PluginPack
{
	...
	{
		private readonly ComponentInformation _info = new ComponentInformation( "Multilanguage Content Plugin", "Threeplicate Srl", "3.0.2.473", "", "" );
		...
		public string Format( string raw, ContextInformation context, FormattingPhase phase )
		{
			...
			if ( result == null )
			{
				result = ExtractLocalizedContent( _defaultLanguage, raw ); // Load content in the default language
				notLocalized = true;
			}
			if ( result == null )
			{
			...
			if ( _displayWarning && !noLocalization && context.Page != null )
			{
		...
		public string PrepareTitle( string title, ContextInformation context )
		{
			...
			{
				result = ExtractLocalizedContent( _defaultLanguage, title ); // Load content in the default language
			}
			if ( result == null )
			{
