...
namespace ScrewTurn.Wiki
{
	...
	public class PagesStorageProvider : ProviderBase, IPagesStorageProviderV30
	{
		...
		private readonly ComponentInformation _info =
			new ComponentInformation( "Local Pages Provider",
		...
		private bool NamespaceExists( string name )
		{
			...
			return FindNamespace( name, allNamespaces ) != null;
		}
		...
		public bool RemoveNamespace( NamespaceInfo nspace )
		{
			...
			lock ( this )
			{
				...
				int findIndex = allNamespaces.FindIndex( x => nspace.CompareTo( x ) == 0 );
				...
				if ( findIndex >= 0 )
				{
					...
					allNamespaces.RemoveAt( findIndex );
		...
		public PageInfo MovePage( PageInfo page, NamespaceInfo destination, bool copyCategories )
		{
			...
			if ( ( currentNs == null && destination == null
				? 0 //0 if both X and Y are null (equal)
				: ( currentNs == null
					? -1 //-1 if just x is null (x is less than y)
					: ( destination == null
						? 1 //1 if just y is null (x is greater than y)
						: StringComparer.OrdinalIgnoreCase.Compare( currentNs.Name, destination.Name ) ) ) ) == 0 )
				return null;
		...
		public CategoryInfo MergeCategories( CategoryInfo source, CategoryInfo destination )
		{
			...
			lock ( this )
			{
				...
				if ( ( sourceNs == null && destinationNs == null
					? 0 //0 if both X and Y are null (equal)
					: ( sourceNs == null
						? -1 //-1 if just x is null (x is less than y)
						: ( destinationNs == null
							? 1 //1 if just y is null (x is greater than y)
							: StringComparer.OrdinalIgnoreCase.Compare( sourceNs.Name, destinationNs.Name ) ) ) ) != 0 )
				{
