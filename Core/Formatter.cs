namespace ScrewTurn.Wiki
{
	...
	public static class Formatter
	{
		...
		private static readonly Regex SpecialTagRegex =
			new Regex( @"\{(wikititle|wikiversion|mainurl|rsspage|themepath|clear|top|searchbox|pagecount|pagecount\(\*\)|categories|cloud|orphans|wanted|namespacelist)\}",
		...
		private static readonly Regex Phase3SpecialTagRegex = new Regex( @"\{(username|pagename|loginlogout|namespace|namespacedropdown|incoming|outgoing)\}",
																		 RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant );
		...
		// Singleline to matche list elements on multiple lines
						...
						if ( !forIndexing )
						{
							switch ( match.Value.Substring( 1, match.Value.Length - 2 ).ToUpperInvariant( ) )
							{
								...
								case "SEARCHBOX":
									...
									string doSearchFunction = "<nowiki><nobr><script type=\"text/javascript\"><!--\r\n" + @"function _DoSearch_" + textBoxId + "() { document.location = '" + nsstring +
															  @"?AllNamespaces=1&FilesAndAttachments=1&Query=' + encodeURI(document.getElementById('" + textBoxId + "').value); }" + "\r\n// -->\r\n</script>";
									sb.Insert( match.Index, doSearchFunction +
															@"<input class=""txtsearchbox"" type=""text"" id=""" + textBoxId +
															@""" onkeydown=""javascript:var keycode; if(window.event) keycode = event.keyCode; else keycode = event.which; if(keycode == 10 || keycode == 13) { _DoSearch_" +
															textBoxId + @"(); return false; }"" /> <big><a href=""#"" onclick=""javascript:_DoSearch_" + textBoxId + @"(); return false;"">&raquo;</a></big></nowiki></nobr>" );
									break;
								case "CATEGORIES":
									...
									string categories = "<ul><li>" + string.Join( "</li><li>",
																				  ( from c in cats
																					select "<a href=\"" + pageName + Tools.UrlEncode( c.FullName ) + "\">" + NameTools.GetLocalName( c.FullName ) + "</a>" ).ToArray( ) ) +
														"</li></ul>";
			...
			while ( match.Success )
			{
				if ( !IsNoWikied( match.Index, noWikiBegin, noWikiEnd, out end ) )
				{
					...
					try
					{
						...
						for ( int i = tempLines.Count - 1; i >= 1; i-- )
						{
							// Skip first line
		...
		private static void BuildHeaderAnchor( StringBuilder buffer, string id )
		{
			...
			{
				buffer.Append( @""" style=""visibility: hidden;"">&nbsp;</a>" );
			}
		}
		...
		private static string BuildIndent( string indent )
		{
			...
			return @"<div class=""indent"" style=""margin: 0px; padding: 0px; padding-left: " + colons * 15 + @"px"">" + indent + "</div>";
		}
		...
		private static string BuildCloud( NamespaceInfo currentNamespace )
		{
			...
			for ( int i = 0; i < categories.Count; i++ )
			{
				if ( categories[ i ].Pages.Length > 0 )
				{
					...
					sb.Append( ComputeSize( categories[ i ].Pages.Length / (float)tot * 100F ).ToString( ) );
					sb.Append( @"px;"">" );
		...
		public static string FormatPhase3( string raw, FormattingContext context, PageInfo current )
		{
			...
			dummy.Append(
				@"</b><span id=""ExpandTocSpan""> [<a href=""#"" onclick=""javascript:if(document.getElementById('Toc').style['display']=='none') document.getElementById('Toc').style['display']=''; else document.getElementById('Toc').style['display']='none'; return false;"">" );
			...
			while ( match.Success )
			{
				...
				try
				{
					...
					string[ ] fields = { txt.Substring( 0, idx ), txt.Substring( idx + 1 ) };
					dummy = new StringBuilder( );
